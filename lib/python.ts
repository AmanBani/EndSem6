    ...
    try {
      ...
      while (i < lines.length) {
        ...
        try {
          ...
          error += `Line ${i + 1}: ${lineError.message || lineError}\n`;
        }
  ...
  private static async executeLine(
    ...
    if (line.includes("print(")) {
      ...
      if (printMatch) {
        ...
        outputCallback(content.toString().replace(/\\n/g, "\n") + "\n");
      }
    }
  }
  ...
  private static evaluateExpression(expr: string, variables: any, functions: any): any {
    ...
    if (funcCallMatch) {
      ...
      const args = funcCallMatch[2]
        .split(",")
        .map((arg) => this.evaluateExpression(arg.trim(), variables, functions));
